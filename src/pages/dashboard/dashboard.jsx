...
const DashboardManagement = () => {
  ...
  // Verificar si el usuario es administrador (rol con ID 1)
    ...
    const fetchData = async () => {
      try {
        ...
        if (ventas && Array.isArray(ventas)) {
          // Si no es admin, filtrar solo las ventas del usuario actual
          if (!isAdmin && user?.id) {
            const ventasFiltradas = ventas.filter((venta) => venta.usuario_id === user.id)
        ...
        // Obtenemos estadísticas - solo para administradores
        if (isAdmin) {
      ...
      <div className="mb-6 bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        {user && (
          <div className="bg-gray-700 p-4 rounded-lg">
            ...
            <p className="text-gray-300">Rol: {user.id_rol === 1 ? "Administrador" : "Empleado"}</p>
          </div>
        )}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Gráfico Dona - visible para todos */}
        <div className="md:col-span-2">
          <DoughnutChart />
        </div>

        {/* Panel de información - solo visible para administradores */}
        {isAdmin && (
            ...
            {cargando ? (
              ...
              <div className="space-y-4">
                {estadisticas.length > 0 &&
                  estadisticas.map((estMes, index) => (
                    <div key={index} className="bg-gray-700 p-4 rounded-lg">
                      <h3 className="text-gray-300 text-sm">
                        {estMes.mes} {estMes.año}
                      </h3>
        ...
        {/* Mensaje para empleados donde estaría la información general */}
        {!isAdmin && (
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold text-white mb-4">Información General</h2>
            <p className="text-gray-300">La información detallada solo está disponible para administradores.</p>
          </div>
        )}
      </div>

      {/* Ventas Recientes - filtradas por usuario si no es admin */}
      <div className="mt-6 bg-gray-800 p-6 rounded-lg shadow-lg">
        <h2 className="text-xl font-bold text-white mb-4">{isAdmin ? "Ventas Recientes" : "Mis Ventas Recientes"}</h2>
              ...
              <tbody className="bg-gray-700 divide-y divide-gray-600">
                {ventasRecientes.map((venta) => (
                  <tr key={venta.id}>
                    ...
                    {isAdmin && <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{venta.usuario_id}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
